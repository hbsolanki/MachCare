...
function UserVehicleRegistration() {
  ...
  const addNewVehicle = async (data) => {
        try {
            const res = await axios.post("http://localhost:8080/user/vehicle/new", data);
        } catch (error) {
            console.log(error);
        }
  };
  ...
  return (
    <div className="max-w-md mx-auto bg-blue-200 p-10 rounded-lg shadow-md mt-10">
      <h2 className="text-xl font-bold text-gray-700 mb-4">Vehicle Registration</h2>
      ...
      <form onSubmit={handleSubmit(addNewVehicle)} className="space-y-4">
        ...
        <div>
          ...
          <select
            id="fuelType"
            {...register("fuelType", { required: "Please select type of fuel" })}
            className="mt-1 block w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
          >
        ...
        <div>
          ...
          <input
            ...
            {...register("vehicleNo", { required: "Please enter vehicle number",
                pattern: {
            ...
            className="mt-1 block w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
          />
          {errors.vehicleNo && <p className="text-red-500 text-sm mt-1">{errors.vehicleNo.message}</p>}
        </div>
        ...
        <div>
          ...
          <input
            ...
            type="number"
            {...register("manufacturingYear", { required: "Please enter manufacturing year" })}
            className="mt-1 block w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
          />
          {errors.manufacturingYear && <p className="text-red-500 text-sm mt-1">{errors.manufacturingYear.message}</p>}
        </div>
